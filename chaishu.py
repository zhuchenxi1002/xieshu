...
"""
    ...
    def _auto_fill_book_info(self, book_title, intro_content):
        ...
        # 设置新书简介
        self.new_book_synopsis.set(intro_content[:500])  # 限制长度
...
"""
    ...
    def create_write_page1(self):
        ...
        gen_btn = ttk.Button(left_frame, text="生成新书提示词", command=self.generate_prompt_from_selected)
        ...
        save_btn = ttk.Button(right_frame, text="保存提示词", command=self.save_prompt)
    ...
    def create_write_page2(self):
        ...
        ttk.Button(btn_frame, text="从模板库加载", command=self.load_template_from_library).pack(side=tk.LEFT, padx=5, pady=5)
    ...
    def gen_architecture(self):
        """生成整体架构，实现jiagou文件夹逻辑"""
        # 二、jiagou文件夹（架构相关）操作逻辑
        # 1. 触发条件：点击「生成整体架构」功能按钮时，触发该检测和执行逻辑
        
        if not self.new_book_title.get().strip():
            messagebox.showwarning("提示", "请先填写新书名")
            return
        
        # 2. 检测目标：「新书名」文件夹→jiagou文件夹中，是否存在「书名-架构」文档
        ...
        # 检查架构文档是否存在
        if os.path.exists(arch_file):
            # 3. 执行动作：若存在「书名-架构」文档，弹出对话窗
            response = messagebox.askyesno("架构已存在", 
                f"架构文档已存在：{arch_file}\n\n是否要重新生成架构？\n\n"
                "点击「是」重新生成架构（将覆盖原有内容）\n"
                "点击「否」停止任何更改")
            
            if not response:  # 用户点击「否」
...
请确保架构完整、逻辑清晰，可直接用于后续创作。"""
    ...
    def gen_blueprints(self):
        """生成章节蓝图，实现lantu文件夹逻辑"""
        # 三、lantu文件夹（蓝图相关）操作逻辑
        # 1. 触发条件：点击「章节蓝图」功能按钮时，触发该检测和执行逻辑
        
        if not self.architecture:
            messagebox.showwarning("提示", "请先执行Step1生成架构")
            return
        
        # 2. 检测目标：「新书名」文件夹→lantu文件夹中，是否存在「书名-蓝图」文档，且文档已完成
        book_title = self.new_book_title.get().strip()
        save_dir = self.save_path.get()
        book_dir = os.path.join(save_dir, book_title)
        ...
        if os.path.exists(blueprint_file):
            ...
            try:
                ...
                # 判断文档是否已完成（这里简单判断是否有足够的内容）
                if len(existing_content) > 1000:
                    # 3. 执行动作：若文档已完成，无需进行任何书写、修改操作
                    ...
                    # 文档未完成，继续生成
                    self.log_to_write(f"蓝图文档存在但未完成，继续生成...")
            except Exception as e:
...
- 本章的节奏控制策略：____________"""
            ...
            try:
                ...
                self.log_to_write(f"保存蓝图文档失败：{e}")
    ...
    def gen_all_drafts(self):
        """生成全部草稿，实现caogao文件夹逻辑"""
        # 四、caogao文件夹（草稿相关）操作逻辑
        # 1. 触发条件：点击「生成全部草稿」功能按钮时，触发该检测和执行逻辑
        
        if not self.chapter_blueprints:
            messagebox.showwarning("提示", "请先执行Step2生成蓝图")
            return
        
        # 2. 检测目标：「新书名」文件夹→caogao文件夹中，是否存在已生成的章节草稿
        book_title = self.new_book_title.get().strip()
        save_dir = self.save_path.get()
        book_dir = os.path.join(save_dir, book_title)
